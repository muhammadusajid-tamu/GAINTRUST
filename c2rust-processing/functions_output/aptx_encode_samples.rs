...
extern "C" {
    fn aptx_encode_channel(
        channel: *mut aptx_channel,
        samples: *const int32_t,
        hd: libc::c_int,
    );
    fn aptx_invert_quantize_and_prediction(channel: *mut aptx_channel, hd: libc::c_int);
    fn aptx_insert_sync(channels: *mut aptx_channel, sync_idx: *mut uint8_t);
    fn aptx_pack_codeword(channel: *const aptx_channel) -> uint16_t;
    fn aptxhd_pack_codeword(channel: *const aptx_channel) -> uint32_t;
}
...
#[no_mangle]
pub unsafe extern "C" fn aptx_encode_samples(
    ...
    aptx_insert_sync(((*ctx).channels).as_mut_ptr(), &mut (*ctx).sync_idx);
    channel = 0 as libc::c_int as libc::c_uint;
    while channel < NB_CHANNELS as libc::c_int as libc::c_uint {
        ...
        if (*ctx).hd != 0 {
            ...
            *output
                ...
                ) = (codeword >> 16 as libc::c_int & 0xff as libc::c_int as libc::c_uint)
                as uint8_t;
            *output
                .offset(
                    (3 as libc::c_int as libc::c_uint)
                ...
                ) = (codeword >> 8 as libc::c_int & 0xff as libc::c_int as libc::c_uint)
                as uint8_t;
            *output
                .offset(
                    (3 as libc::c_int as libc::c_uint)
                ...
                ) = (codeword >> 0 as libc::c_int & 0xff as libc::c_int as libc::c_uint)
                as uint8_t;
        } else {
