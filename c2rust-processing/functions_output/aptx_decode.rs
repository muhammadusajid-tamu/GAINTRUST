...
pub unsafe extern "C" fn aptx_decode(
            ...
            || (*ctx).decode_skip_leading as libc::c_int > 0 as libc::c_int)
    {
            ...
            17216689946888361452 => {
                while sample < 4 as libc::c_int as libc::c_uint {
                    ...
                    while channel < NB_CHANNELS as libc::c_int as libc::c_uint {
                        *output
                            .offset(
                                opos.wrapping_add(0 as libc::c_int as libc::c_ulong)
                                    as isize,
                        ...
                        *output
                            .offset(
                                opos.wrapping_add(1 as libc::c_int as libc::c_ulong)
                                    as isize,
                            ) = (samples[channel as usize][sample as usize] as uint32_t
                        ...
                        *output
                            .offset(
                                opos.wrapping_add(2 as libc::c_int as libc::c_ulong)
                                    as isize,
                            ) = (samples[channel as usize][sample as usize] as uint32_t
        ...
        ipos = (ipos as libc::c_ulong).wrapping_add(sample_size) as size_t as size_t;
    }
    *written = opos;
    return ipos;
}
